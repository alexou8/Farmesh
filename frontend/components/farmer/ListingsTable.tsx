"use client";

import { useState } from "react";
import type { Listing } from "@/types";
import StatusBadge from "@/components/layout/StatusBadge";
import ConfirmDialog from "@/components/common/ConfirmDialog";

type ListingsTableProps = {
    listings: Listing[];
    onDeleteListing?: (listing: Listing) => Promise<void>;
};

export default function ListingsTable({ listings, onDeleteListing }: ListingsTableProps) {
    const [deletingId, setDeletingId] = useState<string | null>(null);
    const [pendingDelete, setPendingDelete] = useState<Listing | null>(null);

    const formatDate = (value?: string) => {
        if (!value) return "—";
        const parsed = new Date(value);
        if (Number.isNaN(parsed.getTime())) return value;
        return parsed.toLocaleDateString();
    };

    const requestDelete = (listing: Listing) => {
        if (!onDeleteListing || deletingId) return;
        setPendingDelete(listing);
    };

    const handleDelete = async () => {
        if (!onDeleteListing || deletingId || !pendingDelete) return;
        try {
            setDeletingId(pendingDelete.id);
            await onDeleteListing(pendingDelete);
            setPendingDelete(null);
        } finally {
            setDeletingId(null);
        }
    };

    const cancelDelete = () => {
        if (deletingId) return;
        setPendingDelete(null);
    };

    return (
        <>
            <div
                className="overflow-x-auto border"
                style={{ borderColor: "var(--border-soft)" }}
            >
                <table className="w-full text-left text-sm">
                    <thead>
                        <tr className="border-b" style={{ borderColor: "var(--border-soft)", backgroundColor: "var(--surface-muted)" }}>
                            {["Product", "Quantity", "Price", "Expires", "Status", "Actions"].map((h) => (
                                <th
                                    key={h}
                                    className="px-5 py-3 text-[11px] font-semibold tracking-[0.15em] uppercase"
                                    style={{ color: "var(--text-muted)" }}
                                >
                                    {h}
                                </th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {listings.map((listing) => {
                            const isExpired = listing.status === "EXPIRED";
                            return (
                                <tr
                                    key={listing.id}
                                    className="border-b last:border-0 transition-colors duration-200"
                                    style={{
                                        borderColor: "var(--border-subtle)",
                                        opacity: isExpired ? 0.68 : 1,
                                        backgroundColor: "var(--surface-base)",
                                    }}
                                    onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "var(--surface-card)")}
                                    onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "var(--surface-base)")}
                                >
                                    <td className="px-5 py-4 font-medium" style={{ color: "var(--foreground)" }}>
                                        {listing.product}
                                    </td>
                                    <td className="px-5 py-4 text-sm" style={{ color: "var(--text-muted)" }}>
                                        {listing.quantity} {listing.unit}
                                    </td>
                                    <td className="px-5 py-4 text-sm" style={{ color: "var(--text-muted)" }}>
                                        ${listing.pricePerUnit.toFixed(2)} / {listing.unit}
                                    </td>
                                    <td className="px-5 py-4 text-sm" style={{ color: "var(--text-subtle)" }}>
                                        {formatDate(listing.expirationDate)}
                                    </td>
                                    <td className="px-5 py-4">
                                        <StatusBadge status={listing.status} />
                                    </td>
                                    <td className="px-5 py-4">
                                        <button
                                            type="button"
                                            onClick={() => requestDelete(listing)}
                                            disabled={!onDeleteListing || Boolean(deletingId)}
                                            className="border px-3 py-1.5 text-[10px] font-semibold tracking-[0.12em] uppercase transition-colors duration-200 disabled:opacity-60"
                                            style={{ borderColor: "#fecaca", color: "#b91c1c", backgroundColor: "#fff1f2" }}
                                        >
                                            {deletingId === listing.id ? "Deleting..." : "Delete"}
                                        </button>
                                    </td>
                                </tr>
                            );
                        })}
                    </tbody>
                </table>
            </div>

            <ConfirmDialog
                open={Boolean(pendingDelete)}
                title="Delete Listing"
                message={
                    pendingDelete
                        ? `Delete "${pendingDelete.product}" from your active listings? This action cannot be undone.`
                        : ""
                }
                confirmLabel="Delete Listing"
                cancelLabel="Cancel"
                accentColor="green"
                busy={Boolean(deletingId)}
                busyLabel="Deleting..."
                onCancel={cancelDelete}
                onConfirm={() => void handleDelete()}
            />
        </>
    );
}
